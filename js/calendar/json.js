...
function getDataByDate(day, month, year, all_categories)
{
	var date = String(day) + String(month) + String(year);
	var result = "<div class=\"modal fade\" tabindex=\"-1\" role=\"dialog\" id=\"modal-" + day + "-" + month + "-" + year + "-" + all_categories + "\"><div class=\"modal-dialog\"><div class=\"modal-content\"><div class=\"modal-header\"><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-label=\"Close\"><span aria-hidden=\"true\">X</span></button><h2>Events am " + day + "." +  ++month + "." + year + "</h2></div><div class=\"modal-body\">";
	for(var i = 0; i < data.length; ++i)
	{
		var data_date = data[i]["date"]["day"] + data[i]["date"]["month"] + data[i]["date"]["year"];
		if(data_date == date)
